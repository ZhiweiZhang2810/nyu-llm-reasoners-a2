...
def run_attention_benchmark(
    ...
    for _ in range(num_warmups):
        out = a1_basics.model.scaled_dot_product_attention(q, k, v)
...
def main():
    ...
    # Table header
    print(f"{'d_model':<8} | {'Seq Len':<8} | {'Fwd (ms)':<10} | {'Bwd (ms)':<10} | {'Peak Mem (MB)':<12}")
    print("-" * 70)
    ...
    for d_model in d_models:
        for seq_len in seq_lengths:
            try:
                ...
                fwd, fwd_s, bwd, bwd_s, mem = run_attention_benchmark(
                    batch_size=batch_size,
                    seq_len=seq_len,
                    d_model=d_model
                )
                print(f"{d_model:<8} | {seq_len:<8} | {fwd:>5.2f}±{fwd_s:<4.2f} | {bwd:>5.2f}±{bwd_s:<4.2f} | {mem:>8.2f}")
                
            except RuntimeError as e:
                if "out of memory" in str(e).lower():
                    print(f"{d_model:<8} | {seq_len:<8} | {'OOM':<10} | {'OOM':<10} | {'OOM':<12}")
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                else:
                    raise e
