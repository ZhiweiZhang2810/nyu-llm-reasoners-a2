...
def benchmark(description: str, run: Callable, num_warmups: int = 5, num_steps: int = 10) -> float:
    ...
    Includes standard deviation calculation and memory profiling.
    """
    ...
    # --- MEMORY PROFILING END & SAVE ---
    if is_cuda:
        # Create a filesystem-safe filename from the description
        safe_desc = "".join([c if c.isalnum() else "_" for c in description])
        filename = f"memory_snapshot_{safe_desc}.pickle"
...
def main():
    ...
    print("- Memory Profiles: .pickle files generated for each successful configuration.")
    print("=" * 60)
